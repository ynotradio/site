<?php
/**
 * Modern Rock Madness Admin Functions
 * 
 * This file contains functions used for the admin management of the Modern Rock Madness tournament.
 * These functions are used only in the admin area and were separated from the main mrm_fns.php file
 * as part of the MVC architecture migration.
 */
...
// These are legacy support functions needed for the display of the match data
// Will be migrated to controllers in future updates
...
function record_ip($match_id, $band_id)
{
    $auth0 = $GLOBALS['auth0'];
    $userInfo = $auth0->getUser();

    if ($userInfo) {
        $email = mysqli_real_escape_string(open_db(), $userInfo['email']);
        $ip = mysqli_real_escape_string(open_db(), $_SERVER['REMOTE_ADDR']);
        $insert = "INSERT INTO mrm_votes VALUES (id, " . $match_id . ", " . $band_id . ", '" . $ip . "', '" . $email . "')";
