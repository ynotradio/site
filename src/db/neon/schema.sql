...
CREATE TABLE votes (
    ...
    sanity_contest_id VARCHAR(255) NOT NULL,
    sanity_option_id VARCHAR(255),
    sanity_category_id VARCHAR(255),
    sanity_matchup_id VARCHAR(255),
    rank INTEGER,
...
CREATE INDEX idx_votes_contest ON votes(sanity_contest_id);
CREATE INDEX idx_votes_option ON votes(sanity_contest_id, sanity_option_id);
CREATE INDEX idx_votes_category ON votes(sanity_contest_id, sanity_category_id);
CREATE INDEX idx_votes_matchup ON votes(sanity_matchup_id);
...
CREATE TABLE contest_entries (
    ...
    submitted_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    UNIQUE(email, sanity_contest_id)
);

CREATE INDEX idx_entries_contest ON contest_entries(sanity_contest_id);
CREATE INDEX idx_entries_winner_pool ON contest_entries(sanity_contest_id, display) WHERE display = TRUE;
...
BEGIN
    ...
    WHERE ce.sanity_contest_id = p_contest_id AND ce.display = TRUE
    ORDER BY RANDOM()
    LIMIT 1;
END;
$$ LANGUAGE plpgsql;
...
RETURNS TABLE (sanity_option_id VARCHAR, sanity_category_id VARCHAR, total_votes BIGINT, weighted_score BIGINT) AS $$
BEGIN
    RETURN QUERY
    SELECT 
        v.sanity_option_id,
    ...
    GROUP BY v.sanity_option_id, v.sanity_category_id;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    SELECT 
        v.write_in_value,
        v.sanity_category_id as category,
        COUNT(*) as count
    FROM votes v
    WHERE v.sanity_contest_id = p_contest_id AND v.is_write_in = TRUE
    GROUP BY v.write_in_value, v.sanity_category_id
    ORDER BY count DESC;
END;
$$ LANGUAGE plpgsql;
