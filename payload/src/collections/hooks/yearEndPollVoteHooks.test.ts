...
describe('enforceUserId', () => {
  it('uses the Auth0 sub claim when available', () => {
    ...
    const result = enforceUserId({
      data,
      req: makeReq({ id: 42, sub: 'auth0|abc' }),
      operation: 'create',
      collection: {} as any,
    });
  ...
  it('does not propagate a client-supplied userId', () => {
    ...
    const result = enforceUserId({
      data,
      req: makeReq({ id: 7, sub: 'auth0|real' }),
      operation: 'create',
      collection: {} as any,
    });
...
describe('rejectDuplicateVote', () => {
  ...
  it('throws when a duplicate vote exists', async () => {
    ...
    await expect(
      rejectDuplicateVote({
        data: { userId: 'u1', nomineeId: 'n1', category: 1 },
        req,
        operation: 'create',
        collection: {} as any,
      }),
...
describe('enforceMaxPicks', () => {
  ...
  it('throws when the voter has reached maxPicks', async () => {
    ...
    await expect(
      enforceMaxPicks({
        data: { userId: 'u1', nomineeId: 'n2', category: 1 },
        req,
        operation: 'create',
        collection: {} as any,
      }),
