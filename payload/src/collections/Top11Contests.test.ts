...
describe('Top11Contests', () => {
  ...
  describe('nominee sort hook', () => {
    ...
    it('orders nominees by artist then title, ignoring a leading "The"', async () => {
      const data = {
        nominees: [{ id: 'a', song: 1 }, { id: 'b', song: 2 }, { id: 'c', song: 3 }, { id: 'd', song: 4 }, { id: 'e', song: { id: 5 } }],
      };

      const result = (await sortHook?.({ data, req } as never)) as { nominees: Array<{ id: string }> };
  ...
  it('hides nominee drag handles since order is derived', () => {
    const nomineesField = Top11Contests.fields.find((field) => 'name' in field && field.name === 'nominees') as {
