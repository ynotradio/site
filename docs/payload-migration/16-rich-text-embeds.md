...
`<iframe>` markup. This is the prerequisite for flipping
...
- One Lexical block (`EmbedFeature`) with `url` + `caption` fields.
...
1. **Legacy embed CSS.** The PHP helper emits `embed`, `embed--video`,
...
2. **Content migration for archetypes D & E (~20 pages).** Convert existing raw
...
3. **UTF-8 / mojibake cleanup.** Separate, data-side concern (the legacy
...
4. **Flag granularity.** `use_postgres_customtext` is currently all-or-nothing
...
5. **Verification.** Render D/E pages from Postgres and compare against the
