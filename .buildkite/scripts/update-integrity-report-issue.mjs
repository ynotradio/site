#!/usr/bin/env node
/**
 * Update a GitHub issue with the latest Content Integrity Report.
 *
 ...
 *   INTEGRITY_REPORT_ISSUE_NUMBER — GitHub issue number to update (required)
 ...
 *
 * Usage: node .buildkite/scripts/update-integrity-report-issue.mjs <report-file>
 */

import { readFileSync } from 'node:fs';

const GITHUB_TOKEN = process.env.GITHUB_PR_TOKEN;
const ISSUE_NUMBER = process.env.INTEGRITY_REPORT_ISSUE_NUMBER;
...
if (!ISSUE_NUMBER) {
  console.error('❌ INTEGRITY_REPORT_ISSUE_NUMBER is not set');
  process.exit(1);
}
...
if (!reportFile) {
  console.error(
    'Usage: node update-integrity-report-issue.mjs <report-file>',
  );
  process.exit(1);
}
...
try {
  ...
  console.error(
    `❌ Could not read report file: ${reportFile}\n${err.message}`,
  );
  process.exit(1);
}
...
const headers = {
  ...
  'User-Agent': 'buildkite-integrity-report',
};
