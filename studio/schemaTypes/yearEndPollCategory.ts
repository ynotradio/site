        ...
        defineArrayMember({
          ...
          fields: [
            defineField({
              ...
              type: 'reference',
              to: [{ type: 'song' }],
              hidden: ({ parent }) => parent?.categoryType !== 'songs',
            }),
            defineField({
              name: 'record',
              title: 'Record',
              type: 'reference',
              to: [{ type: 'record' }],
              hidden: ({ parent }) => parent?.categoryType !== 'records',
            }),
            defineField({
              name: 'artist',
              title: 'Artist',
              type: 'reference',
              to: [{ type: 'artist' }],
              hidden: ({ parent }) => parent?.categoryType !== 'artists',
            }),
            defineField({
              name: 'concert',
              title: 'Concert',
              type: 'reference',
              to: [{ type: 'concert' }],
              hidden: ({ parent }) => parent?.categoryType !== 'concerts',
            }),
            defineField({
              name: 'freeformLabel',
              title: 'Freeform Label',
              type: 'string',
              hidden: ({ parent }) => parent?.categoryType !== 'freeform',
            }),
          ],
          preview: {
            select: {
